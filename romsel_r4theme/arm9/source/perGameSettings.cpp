...
static void checkSaves(std::string filenameForInfo, u32 totalRomSize, u32 pubSize, u32 prvSize) {
	if (isDSiWare && pubSize == 0 && prvSize == 0) return;
	const bool savFormat = !isDSiWare || (totalRomSize >= 0x04000000) || (sys().scfgSdmmcEnabled() && (perGameSettings_dsiWareSlot1Mode == -1 ? DEFAULT_DSIWARE_SLOT1_MODE : perGameSettings_dsiWareSlot1Mode)) || (ms().secondaryDevice && (!isDSiMode() || !sys().scfgSdmmcEnabled() || bs().b4dsMode));

	int saveNoBak = perGameSettings_saveNo;
	for (int i = 0; i < 10; i++) {
		perGameSettings_saveNo = i;
		if (isDSiWare && !savFormat) {

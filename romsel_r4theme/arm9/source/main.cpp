		...
		if (startMenu) {
		  ...
		  if (theme == 6) {
			...
			if ((pressed & KEY_TOUCH) && touch.px >= 236 && touch.px <= 251 && touch.py >= 3 && touch.py <= 23) {
				...
				int err = runNdsFile ("/_nds/TWiLightMenu/manual.srldr", 0, NULL, true, true, false, true, true, false, -1);
				iprintf ("Start failed. Error %i\n", err);
			}
			...
			if (extension(filename, {".nds", ".dsi", ".ids", ".srl", ".app"})) {
				...
				if(useBackend) {
					...
					|| ((game_TID[0] == 'K' || game_TID[0] == 'Z') && romUnitCode > 0)) {
						...
						argarray.at(0) = (char *)ndsToBoot;
						int err = runNdsFile (argarray[0], argarray.size(), (const char **)&argarray[0], (homebrewBootstrap ? false : true), true, false, true, true, false, -1);
						char text[32];
						snprintf (text, sizeof(text), "Start failed. Error %i", err);
						clearText();
						dialogboxHeight = (err==1 ? 2 : 0);

	...
	*fake_heap_end = 0;
	...
	while (1)
	{
				...
				case 2:
					...
					if (useGbarunner)
					{
						...
						else
						{
							...
							int err = 0;
							if (bootstrapFile)
								err = runNdsFile("sd:/_nds/nds-bootstrap-hb-nightly.nds", 0, NULL, true);
							else
								err = runNdsFile("sd:/_nds/nds-bootstrap-hb-release.nds", 0, NULL, true);
							iprintf("Start failed. Error %i\n", err);
						}
					}
					else
		...
		if (applaunch)
		{
			...
			if (!flashcardUsed && isDSiWare && useBootstrap && strcasecmp(filename.c_str() + filename.size() - 4, ".nds") == 0)
			{
			...
			if (strcasecmp(filename.c_str() + filename.size() - 4, ".nds") == 0)
			{
				...
				if (useBootstrap)
				{
					if (!flashcardUsed)
					{
...
#endif
												if (!cheatsFound)
													snprintf(cheatData, sizeof(cheatData), "%08X ", *it);
												else
													snprintf(cheatData, sizeof(cheatData), "%s%08X ", cheatData, *it);
												cheatsFound = true;
#ifdef DEBUG
						...
						if (cheatsFound)
							bootstrapini.SetString("NDS-BOOTSTRAP", "CHEAT_DATA", cheatData);
						else

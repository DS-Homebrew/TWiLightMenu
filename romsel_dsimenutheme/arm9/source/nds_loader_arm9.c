...
int runNds (const void* loader, u32 loaderSize, u32 cluster, bool initDisc, bool dldiPatchNds, bool loadFromRam, const char* filename, int argc, const char** argv, bool clearMasterBright, bool dsModeSwitch, bool lockScfg, bool boostCpu, bool boostVram, int language)
{
	char* argStart;
	u16* argData;
	u16 argTempVal = 0;
...
int runUnlaunchDsi (const char* filename, u32 sector, int argc, const char** argv)  {
	...
	return runNds (load_bin, load_bin_size, sector, true, false, true, filename, argc, argv, true, false, false, true, true, -1);
}
...
int runNdsFile (const char* filename, int argc, const char** argv, bool dldiPatchNds, bool clearMasterBright, bool dsModeSwitch, bool boostCpu, bool boostVram, int language)  {
	...
	return runNds (load_bin, load_bin_size, st.st_ino, true, (dldiPatchNds && memcmp(io_dldi_data->friendlyName, "Default", 7) != 0), loadFromRam, filename, argc, argv, clearMasterBright, dsModeSwitch, lockScfg, boostCpu, boostVram, language);
}

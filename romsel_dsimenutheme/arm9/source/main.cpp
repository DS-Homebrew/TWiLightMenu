...
// Progress screen while a GBA ROM is copied to the Slot-2 cart
...
static void launchWithConfig(const CustomLauncher *launcher, const std::string &romFolder, const std::string &filename, const std::vector<char *> &argarray)
{
	...
	const char *menuSrldr = (sys().isRunFromSD() ? "sd:/_nds/TWiLightMenu/dsimenu.srldr" : "fat:/_nds/TWiLightMenu/dsimenu.srldr");
	...
	while (1) {
		...
		if (applaunch) {
			...
			if (extension(filename, {".nds", ".ndz", ".dsi", ".ids", ".srl", ".app"})) {
				...
				free(argarray.at(0));
				argarray.at(0) = filePath;
				if (!isNdz[CURPOS] && !ms().secondaryDevice && !sys().arm7SCFGLocked() && ms().consoleModel == TWLSettings::EDSiRetail && isHomebrew[CURPOS] && !(perGameSettings_useBootstrap == -1 ? true : perGameSettings_useBootstrap)) {
					ms().romPath[ms().secondaryDevice] = std::string(argarray[0]);
					...
					if (isNdz[CURPOS] || ((perGameSettings_fcGameLoader == -1 ? (ms().fcGameLoader == TWLSettings::EPicoLoader) : (perGameSettings_fcGameLoader == TWLSettings::EPicoLoader)) && !ms().homebrewBootstrap && ms().secondaryDevice && (isDSiMode() || unitCode[CURPOS] < 3))) {

	...
	while (1) {
		...
		if (applaunch) {
			...
			if ((isDSiMode() || sdFound()) && isDSiWare[CURPOS]) {
				...
				ms().dsiWarePubPath = replaceAll(ms().dsiWarePubPath, typeToReplace, (strncmp(gameTid[CURPOS], "Z2E", 3) == 0 && ms().secondaryDevice && (!ms().dsiWareToSD || (isDSiMode() && !sdFound()))) ? getSavExtension() : getPubExtension());
				...
				if ((ms().dsiWareBooter || sys().arm7SCFGLocked() || ms().consoleModel > 0) && !ms().homebrewBootstrap) {
					// Use nds-bootstrap
					loadPerGameSettings(filename);
			...
			if (extension(filename, {".nds", ".dsi", ".ids", ".srl", ".app"})) {
				...
				if (useBackend) {
					...
					|| ((gameTid[CURPOS][0] == 'K' || gameTid[CURPOS][0] == 'Z') && unitCode[CURPOS] > 0)) {
						...
						bootstrapini.SetInt("NDS-BOOTSTRAP", "LANGUAGE", perGameSettings_language == -2 ? ms().gameLanguage : perGameSettings_language);
						bootstrapini.SetInt("NDS-BOOTSTRAP", "REGION", perGameSettings_region == -3 ? ms().gameRegion : perGameSettings_region);
						bootstrapini.SetInt("NDS-BOOTSTRAP", "DSI_MODE", dsModeForced ? 0 : (perGameSettings_dsiMode == -1 ? ms().bstrap_dsiMode : perGameSettings_dsiMode));
						if (dsiFeatures() || !ms().secondaryDevice) {

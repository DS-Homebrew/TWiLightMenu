	...
	while (1) {
		...
		if (applaunch) {
			...
			if (isDSiWare[CURPOS]) {
				...
				ms().dsiWarePubPath = replaceAll(ms().dsiWarePubPath, typeToReplace, (strncmp(gameTid[CURPOS], "Z2E", 3) == 0 && ms().secondaryDevice && (!sdFound() || !ms().dsiWareToSD)) ? getSavExtension() : getPubExtension());

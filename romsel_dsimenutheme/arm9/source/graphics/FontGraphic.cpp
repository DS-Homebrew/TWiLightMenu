...
u8 FontGraphic::textBuf[2][256 * 192]; // Increase to two if adding top screen support
...
FontGraphic::FontGraphic(const char* path, const bool large, const bool useCommonCache, const bool set_useTileCache) {
	// logPrint("FontGraphic::FontGraphic(%s, false)\n\n", path);

	// logPrint("Font path: %s\n", path);
	// logPrint("large: %s\n", large ? "true" : "false");
	// logPrint("useCommonCache: %s\n", useCommonCache ? "true" : "false");
	// logPrint("set_useTileCache: %s\n", set_useTileCache ? "true" : "false");

...
int bipsTexID, scrollwindowTexID, scrollwindowfrontTexID, buttonarrowTexID, startTexID, startbrdTexID, gbamodeTexID, settingsTexID, braceTexID, boxfullTexID, boxemptyTexID;
...
glImage gbamodeImage[(64 / 16) * (64 / 16)];
		...
		{*/
			...
			if(pagenum == 0) {
				if (isRegularDS) {
					...
					glSprite(48-titleboxXpos, titleboxYpos+12, GL_FLIP_NONE, gbamodeImage);
				} else {
					glSprite(32-titleboxXpos, titleboxYpos-1, GL_FLIP_NONE, settingsImage);
				}
			}
			if (theme==0) {
				...
				if(pagenum == 0) {
					if (isRegularDS) {
						...
						glSprite(72-titleboxXpos-64, 80, GL_FLIP_NONE, braceImage);
					}
				} else {
			...
			if (applaunchprep && theme==0) {
				...
				if (cursorPosition == -2) {
					...
					glSprite(112, 96-titleboxYmovepos, GL_FLIP_NONE, gbamodeImage);
				} else {
					glSprite(96, 84-titleboxYmovepos, GL_FLIP_NONE, boxfullImage);
...
void graphicsInit()
{
	...
	if (!isRegularDS && cursorPosition == -2) {
		titleboxXpos = -64;
		titlewindowXpos = -5;
	}
	...
	*(u16*)(0x0400006C) &= BIT(15);
	...
	gbamodeTexID = glLoadTileSet(gbamodeImage, // pointer to glImage array
							32, // sprite width
							32, // sprite height
							32, // bitmap width
							32, // bitmap height

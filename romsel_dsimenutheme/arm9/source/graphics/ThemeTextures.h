...
class ThemeTextures {
	...
	void loadCornerButtonImage(const GritTexture &tex, int arraysize, int sprW, int sprH);
	 ...
	 */
	unique_ptr<glImage[]> loadTexture(int *textureId, const GritTexture &texture, unsigned int arraySize, int sprW,
					  int sprH);
...
inline u16 alphablend(u16 fg, u16 bg, u8 alpha) {
	...
	// Pack the color back in
	return (u16)((out_r << 10) | (out_g << 5) | out_b);

	// return ((val>>10)&0x1f) | ((val)&(0x1f<<5)) | (val&0x1f)<<10 | BIT(15);

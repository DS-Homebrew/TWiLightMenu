...
int subBgTexID, mainBgTexID, bubbleTexID, braceTexID, boxfullTexID;
...
void vBlankHandler()
{
	...
	glBegin2D();
	{
		...
		else
		{
			...
			glSprite(80-titleboxXpos, 84, GL_FLIP_NONE, braceImage);
			int Xpos = 96;
			int iconXpos = 112;
...
void graphicsInit()
{
	...
	braceTexID = glLoadTileSet(braceImage, // pointer to glImage array
							...
							GL_TEXTURE_WRAP_S | GL_TEXTURE_WRAP_T | TEXGEN_OFF | GL_TEXTURE_COLOR0_TRANSPARENT, // param for glTexImage2D() in videoGL.h
							16, // Length of the palette to use (256 colors)
							(u16*) bracePal, // Load our 256 color tiles palette

					...
					case 3:
						...
						if (useGbarunner && gbaBiosFound) {
							if (secondaryDevice) {
								...
								char bootstrapPath[64];
								sprintf(bootstrapPath, "sd:/_nds/nds-bootstrap-hb-%s.nds", bootstrapFile ? "nightly" : "release");

								std::vector<char*> argarray;
								argarray.push_back(strdup(bootstrapPath));
								argarray.at(0) = (char*)bootstrapPath;
		...
		if (applaunch) {
			...
			if (isDSiMode() && isDSiWare) {
				...
				if (consoleModel > 0) {
					...
					printSmall(false, 4, 96, useNightly ? "nds-bootstrap (Nightly)" : "nds-bootstrap (Release)");
			...
			if (extention(filename, ".nds") || extention(filename, ".dsi")
			 ...
			 || extention(filename, ".app")) {
				...
				if (memcmp(game_TID, "HND", 3) == 0 || memcmp(game_TID, "HNE", 3) == 0) {
					...
					loadPerGameSettings(filename);
					useBackend = (perGameSettings_directBoot || (useBootstrap && previousUsedDevice));
					dsModeSwitch = (isDSiMode() && !perGameSettings_dsiMode);
					homebrewBootstrap = true;
				} else {
					loadPerGameSettings(filename);
					useBackend = true;
				...
				std::string ndsToBoot = "sd:/_nds/nds-bootstrap-release.nds";
				if (extention(filename, ".plg")) {
					...
					if (previousUsedDevice) {
						...
						bootstrapini.SetInt("NDS-BOOTSTRAP", "BOOST_CPU", extention(filename, ".smc") || extention(filename, ".sfc") ? 1 : 0);
						bootstrapini.SetInt("NDS-BOOTSTRAP", "BOOST_VRAM", 0);

						if (extention(filename, ".gen") || extention(filename, ".smc") || extention(filename, ".sfc")) {
				...
				argarray.at(0) = (char *)ndsToBoot.c_str();

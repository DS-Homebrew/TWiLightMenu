...
int runUnlaunchDsi (const char* filename, u32 sector)  {
	...
	return runNds (load_bin, load_bin_size, sector, false, false, true, 0, NULL, true, false, false, true, true, -1);
}
...
int runNdsFile (const char* filename, int argc, const char** argv, bool dldiPatchNds, bool clearMasterBright, bool dsModeSwitch, bool boostCpu, bool boostVram, int language)  {
	...
	return runNds (load_bin, load_bin_size, st.st_ino, (memcmp(io_dldi_data->friendlyName, "CycloDS iEvolution", 18) != 0), (dldiPatchNds && memcmp(io_dldi_data->friendlyName, "Default", 7) != 0), loadFromRam, argc, argv, clearMasterBright, dsModeSwitch, lockScfg, boostCpu, boostVram, language);
}

...
void MainWnd::bootBootstrap(PerGameSettings &gameConfig, DSRomInfo &rominfo)
{
	...
	if (settingsIni.checkIfShowAPMsg() && (access(ipsPath, F_OK) != 0)) {
		...
		if ((memcmp(rominfo.saveInfo().gameCode, "VETP", 4) == 0)	// 1000 Cooking Recipes from Elle a Table (Europe)
		 ...
		 || (memcmp(rominfo.saveInfo().gameCode, "BA5P", 4) == 0)	// Mario & Luigi RPG: Siganui Partner (Korea)
...
void MainWnd::launchSelected()
{
    ...
    // Launch DSiWare
    if (!rominfo.isHomebrew() && rominfo.isDSiWare() && isDSiMode() && ms().consoleModel == 0 && !ms().dsiWareBooter)
    {
        // Unlaunch boot here....

...
SmallClock::SmallClock() : Window(NULL, "small clock") {
	_size = Size(0, 0);
...
void SmallClock::drawNumber(u8 id, u8 number) {
	if (number > 10)
		return;
...
void SmallClock::drawColon() {
	u8 x = _position.x + 2 * _numbers.width();
	if (_colon.valid())
	{
		gdi().maskBlt(_colon.buffer(), x - 1, _position.y, _colon.width(), _colon.height(), selectedEngine());
	}
}

void SmallClock::draw()
{
	if (!_show)
		return;

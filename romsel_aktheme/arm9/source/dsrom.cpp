...
bool DSRomInfo::loadDSRomInfo(const std::string &filename, bool loadBanner)
{
    ...
    if (1 != fread(&header, sizeof(header), 1, f))
    {
		if (1 != fread(&header, 0x160, 1, f))
		{
            ...
            tonccpy(_banner.title, ((tNDSBanner*)unknown_nds_banner_bin)->titles[LANG("font", "language")], sizeof(_banner.title));
			fclose(f);
			return false;
		}
    }
    ...
    if (crc != header.headerCRC16)
    {
        ...
        tonccpy(_banner.icon,((tNDSBanner*)unknown_nds_banner_bin)->icon, sizeof(_banner.icon));
        tonccpy(_banner.palette,((tNDSBanner*)unknown_nds_banner_bin)->palette, sizeof(_banner.palette));
        tonccpy(_banner.title, ((tNDSBanner*)unknown_nds_banner_bin)->titles[LANG("font", "language")], sizeof(_banner.title));        fclose(f);
        return true;
    }
    else
    {
    ...
    if (header.bannerOffset != 0)
    {
        ...
        if ((fseek(f, header.bannerOffset, SEEK_SET) == 0 && (bannerSize = fread(&banner, 1, sizeof(banner), f)) == sizeof(banner)) || (fseek(f, header.bannerOffset, SEEK_SET) == 0 && (bannerSize = fread(&banner, 1, NDS_BANNER_SIZE_ORIGINAL, f)) == NDS_BANNER_SIZE_ORIGINAL))
        {
            ...
            tonccpy(_banner.title, &banner.titles[LANG("font", "language")], sizeof(_banner.title));
        }
        else
        {
            ...
            tonccpy(_banner.title, ((tNDSBanner*)nds_banner_bin)->titles[LANG("font", "language")], sizeof(_banner.title));
        }
    }
    else
    {
...
bool DSRomInfo::loadGbaRomInfo(const std::string &filename)
{
    ...
    if (gbaFile)
    {
        ...
        if (header.is96h == 0x96)
        {
            ...
            tonccpy(_banner.title, ((tNDSBanner*)gbarom_banner_bin)->titles[LANG("font", "language")], sizeof(_banner.title));
            return true;
        }
    }
    return false;
...
void DSRomInfo::setBanner(const std::string &anExtIcon, const u8 *aBanner)
{
    ...
    tonccpy(_banner.title, ((tNDSBanner*)aBanner)->titles[LANG("font", "language")], sizeof(_banner.title));
}

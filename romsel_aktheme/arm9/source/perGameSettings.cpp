...
void perGameSettings (std::string filename) {
	...
	if (isHomebrew[cursorPosOnScreen]) {		// Per-game settings for homebrew
		...
		if (bootstrapEnabled) {
			if (((dsiFeatures() && !bs().b4dsMode) || !ms().secondaryDevice) && romUnitCode[cursorPosOnScreen] < 3 && !blacklisted_cardReadDma) {
				perGameOps++;
				perGameOp[perGameOps] = 5;	// Card Read DMA
			}
			if (!ms().secondaryDevice && (romSize > (romUnitCode[cursorPosOnScreen] > 0 ? romSizeLimitTwl : romSizeLimit)) && !blacklisted_asyncCardRead) {
				perGameOps++;
	...
	while (1) {
		...
		switch (perGameOp[i]) {
			...
			case 5:
				...
				if (romUnitCode[cursorPosOnScreen] > 0 && (perGameSettings_dsiMode==-1 ? DEFAULT_DSI_MODE : perGameSettings_dsiMode > 0)) {
			...
			if (held & KEY_LEFT) {
				switch (perGameOp[perGameSettings_cursorPosition]) {
					...
					case 4:
						...
						break;
					case 5:
						if (romUnitCode[cursorPosOnScreen] == 0 || (perGameSettings_dsiMode==-1 ? (DEFAULT_DSI_MODE == TWLSettings::EDSMode) : perGameSettings_dsiMode < 1)) {

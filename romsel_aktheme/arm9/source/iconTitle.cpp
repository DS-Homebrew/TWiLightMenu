...
void getGameInfo(int num, int fileOffset, bool isDir, const char* name, bool fromArgv)
{
	...
	if ((ms().showCustomIcons || bannerFromLauncher) && !preloadedBannerIconFound(fileOffset) && customIcon[num] < 2 && (!fromArgv || customIcon[num] <= 0)) {
	...
	if (isDir) {
		...
		if (preloadedBannerIconFound(fileOffset)) {
			...
			if (customIcon[num] != 2 && !isNdz[num] && ndsHeader.bannerOffset == 0) {
				fclose(fp);

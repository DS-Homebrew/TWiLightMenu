...
int mainTitleTexID, subTitleTexID;
int mainBgTexID, subBgTexID;
glImage topTitleImage[(256 / 16) * (256 / 16)];
glImage subTitleImage[(256 / 16) * (256 / 16)];
...
void vBlankHandler()
{
	...
	if (renderScreens) {
		...
		glBegin2D();
		{
			if (renderingTop)
			{
				if (screenmode == 1) drawBG(topBgImage);
				else drawBG(topTitleImage);
				glColor(RGB15(31, 31, 31));
				updateText(renderingTop);
			}
			else
			{
				if (screenmode == 1) drawBG(subBgImage);
				else if (!flashcardUsed) drawBG(subTitleImage);
				else glBoxFilled(0, 0, 256, 192, 0x7FFF);	// R: 255, G: 255, B: 255
				glColor(RGB15(31, 31, 31));
				updateText(renderingTop);
			}
		}
...
void graphicsInit()
{
	...
	*(u16*)(0x0400006C) &= BIT(15);
	...
	subTitleTexID = glLoadTileSet(subTitleImage, // pointer to glImage array
							16, // sprite width
							16, // sprite height
							256, // bitmap width
							256, // bitmap height
							...
							(u16*) title_subPal, // Load our 256 color tiles palette
							(u8*) title_subBitmap // image data generated by GRIT
							);

	mainBgTexID = glLoadTileSet(topTitleImage, // pointer to glImage array
							16, // sprite width
							16, // sprite height
							256, // bitmap width
							256, // bitmap height
							...
							GL_TEXTURE_WRAP_S | GL_TEXTURE_WRAP_T | TEXGEN_OFF, // param for glTexImage2D() in videoGL.h
							16, // Length of the palette to use (256 colors)
							(u16*) title_topPal, // Load our 256 color tiles palette
							(u8*) title_topBitmap // image data generated by GRIT
							);

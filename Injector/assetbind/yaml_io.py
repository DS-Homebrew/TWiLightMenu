"""
yaml_io.py -- reads/writes the manifest .yml.

Uses PyYAML if it is installed (it handles manual edits well). Otherwise falls back to a
purpose-built reader/writer restricted to this project's schema (version / games[] with
identity{} and assets{}). The round-trip of the emitted format is covered by the tests.
"""
import re
...
def _q(s):
    """Quoting (required for paths with spaces and for hashes)."""
    s = str(s).replace("\\", "\\\\").replace('"', '\\"')
    return f'"{s}"'


def dump_manifest(m):
    """Serializes the manifest dict into deterministic YAML text."""
...
def dump_assets_index(roms):
    """
    Serializes assets_index.yml: map of ROM base name (no extension) -> game_id.
    This is the runtime index the DS uses to match the focused ROM (by name) and load
    assets/<game_id>/. Includes ALL names (duplicates point at the same game_id).
...
def _load_scoped(text):
    m = {"version": 1, "games": []}
    cur = None   # current game entry
    sub = None   # current nested map (identity/assets)
...
def _parse_quoted_key_line(line):
    """Parses `"key with spaces": value` (key in quotes). Returns (key, value)."""
    if line.startswith('"'):
        end = 1
        while end < len(line):
            if line[end] == "\\":
...
def load_assets_index_text(text):
    """Reads assets_index.yml: {version, roms: {name: game_id}}."""
    try:
        import yaml  # type: ignore
        return yaml.safe_load(text)
    except ImportError:

...
STRING(INT_ROMS, "Intellivision ROMs")
...
STRING(DESCRIPTION_SHOW_INT, "Display Intellivision ROMs in the ROM list.")
...
STRING(VRAMBOOST, "VRAM Mode")
STRING(SYSSD_VRAMBOOST, "Sys SD: VRAM Mode")
...
STRING(CARDREADDMA, "Card Read DMA")
STRING(ASYNCHCARDREAD, "Asynch Card Read")
STRING(SYSSD_CARDREADDMA, "Sys SD: Card Read DMA")
STRING(SYSSD_ASYNCHCARDREAD, "Sys SD: Asynch Card Read")
STRING(SWIHALTHOOK, "SWI Halt Hook")
...
STRING(DESCRIPTION_CPUSPEED_1, "Set to TWL for fewer slowdowns, at the cost of a few bugs in some NTR titles.")
...
STRING(DESCRIPTION_CARDREADDMA, "Games use DMA to do faster read operations. Turn this off to work around crashes, at the cost of some game compatibility.")
STRING(DESCRIPTION_ASYNCHCARDREAD, "Run threaded operations while running card read operations. Will not work with some games. Turn this off to fix some crashes.")
STRING(DESCRIPTION_SWIHALTHOOK, "Hooks into SWI Halt for console SD R/W operations, and/or DMA reads from RAM. Does not affect TWL titles in DSi mode. Turn this off to fix slowdown in some games.")
...
STRING(DESCRIPTION_DSIWARETOSD, "Copies the launched DSiWare from flashcard to a temporary location on the console's SD card. Turning this off will increase load speed, but will decrease compatibility.")
...
STRING(DESCRIPTION_GAMEREGION_DS, "Set region for recent homebrew.")
...
STRING(DESCRIPTION_LIMITEDMODE, "Runs TWiLight Menu++ as if your console is in a DSiWarehax environment. To disable it, reboot after doing so, to take effect.")
